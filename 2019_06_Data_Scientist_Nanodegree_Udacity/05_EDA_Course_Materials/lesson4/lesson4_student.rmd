...
pf.fc_by_age <- summarise <- summarise(age_groups,
...
with(Mitchell, dcor.test(Month, Temp))
...
pf.fc_by_agemonths <- pf %>% group_by(age_with_months) %>% summarise(mean = mean(friend_count),
                                               median = median(friend_count),
                                               n = n()) %>% arrange(age_with_months)

head(pf.fc_by_agemonths)
...
ggplot(aes(x = age_with_months, y = mean), data = subset(pf.fc_by_agemonths, age_with_months < 71))+
  geom_line()
...
p2 <- ggplot(aes(x = age_with_months, y = mean), data = subset(pf.fc_by_agemonths, age_with_months < 71))+
  geom_line() +
  geom_smooth()
